...
async def generate_interpretations():
    ...
    print("Starting to generate interpretations for dreams 11-32...\n")

    for i, dream in enumerate(dreams_data, 1):
        print(f"[{i}/22] Dream {dream['id']}...")
    ...
    # 保存到文件
    output_file = Path(__file__).parent / "interpretations_11_32.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"\n[COMPLETE] Saved to: {output_file}")
    print(f"[STATS] Success: {sum(1 for v in results.values() if v is not None)}/22")
